...
' Author: Knut Jetlund
' Purpose: Recursive loop through subpackages and their elements, with controll of dependencies
' Date: 20210115
...
sub getDependency
	...
	if not lstPck.Contains(dtP.PackageGUID) and dtP.PackageGUID <> currentPck.PackageGUID then
		...
		For each pckConnector in currentPck.Connectors
			if pckConnector.Type="Dependency" AND pckConnector.SupplierID = dtP.Element.ElementID then 
		...
		if (el.Type="Class" or el.Type="Interface") and Ucase(el.Stereotype) <> "CODELIST" and UCase(el.Stereotype) <> "ENUMERATION" and UCase(el.Stereotype) <> "UNION" then
			...
			for each attr in el.Attributes
				...
				if attr.ClassifierID <> 0 then
					...
					Repository.WriteOutput "Elements", Now & " |" & p.Name & "|" & el.Stereotype & "|" & el.Name & "|Attribute|" &  attr.Name & "|" & elID & "|", 0
					getDependency()
				else
			...
			for each con in el.Connectors
				...
				If not elID = 0 and elID <> el.ElementID then
					Repository.WriteOutput "Elements", Now & " |" & p.Name & "|" & el.Stereotype & "|" & el.Name & "|" & con.Type & "||" & elID & "|", 0
...
sub recDependencyDiagrams(p)
	...
	For i = 0 to p.Diagrams.Count - 1
		...
		If eDiagram.Name = "Generated package dependencies" Then
		...
		p.Diagrams.Refresh
	Next
	set pckDiagram = p.Diagrams.AddNew("Generated package dependencies", "Package")
	...
	'Loop for registered dependencies - add all to diagram	
