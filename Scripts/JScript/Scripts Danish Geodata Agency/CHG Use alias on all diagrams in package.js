...
function main() {
	...
	if (aPackage != null && aPackage.ParentID != 0)	{
		...
		var currentExtendedStyle, newExtendedStyle;
		var regExp = new RegExp('UseAlias=0', '');
		for (var i = 0; i < diagrams.length; i++) {
			diagram = diagrams[i];
			currentExtendedStyle = diagram.ExtendedStyle;
			LOGDebug("currentExtendedStyle:" + currentExtendedStyle);
			newExtendedStyle = currentExtendedStyle.replace(regExp, 'UseAlias=1');
			LOGDebug("newExtendedStyle:" + newExtendedStyle);
			diagram.ExtendedStyle = newExtendedStyle;
			diagram.Update();
		}
		LOGInfo("Done!");
	}
